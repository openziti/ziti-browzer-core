...
class ZitiBrowzerCore {
  ...
  constructor (options = {}) {
    ...
    this._reporters = options.reporters || [new ZitiReporter()]
    this._types = options.types || Types
    this._logLevel = normalizeLogLevel(options.logLevel, this._types)
    this._domain = 'https://local-controller:1280'
  ...
  get context () {
    if (this._zitiContext === undefined) {
      this._zitiContext = new ZitiContext();
    }
    return this._zitiContext;
  }
